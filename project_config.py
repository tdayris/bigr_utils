...
@click.option("-s", "--samples", type=click.Path(), default=f"{os.getcwd()}/config/samples.csv", help="Path to `samples.csv` file")
@click.option("-g", "--genomes", type=click.Path(), default=f"{os.getcwd()}/config/genomes.csv", help="Path to `genomes.csv` file")
@click.option("-o", "--output",  type=click.Path(), default=f"{os.getcwd()}/config/config.yaml", help="Path to output file")
@click.option("-w", "--workflow", type=click.Path(), default=f"{os.getcwd()}/workflow/Snakefile", help="Path to the snakefile")
@click.option("-f", "--force", is_flag=True, help="Force over-writing", default=False)
@click.option("-v", "--verbose", is_flag=True, help="Raise verbosity level", default=False)
...
def create_config(
    ...
    with open(workflow, "r") as snakefile_stream:
        for line in snakefile_stream:
            ...
            console.print(f":warning: warning: Could not find pipeline version", style="dark_orange")
    ...
    if force or (not Path(output).exists):
        with open(output, "w") as yaml_stream:
            ...
            yaml_stream.write("#params:\n    # ...\n")

    if verbose:
        console.print(":ballot_box_with_check: Configuration file available", style="green")
